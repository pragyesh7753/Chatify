...
var require_follow_redirects = __commonJS({
  "node_modules/follow-redirects/index.js"(exports2, module2) {
    ...
    RedirectableRequest.prototype.write = function(data, encoding, callback) {
      ...
      if (!isString4(data) && !isBuffer3(data)) {
...
var throttle2 = (fn, timeout = 200, { leading = true, trailing = false } = {}) => {
  let runningTimeout = null;
  let storedArgs = null;
  return (...args) => {
    if (runningTimeout) {
...
var DEFAULT_LINK_PREVIEW_MANAGER_CONFIG = {
  ...
  findURLFn: (text) => (0, import_linkifyjs.find)(text, "url", { defaultProtocol: "https" }).reduce((acc, link) => {
    try {
      ...
      if (link.isLink && /^[a-zA-Z0-9-.]+\.[a-zA-Z]{2,}$/.test(url2.hostname)) {
        acc.push(link.href);
      }
    } catch {
    }
...
var _LinkPreviewsManager = class _LinkPreviewsManager {
  constructor({ composer, message }) {
    ...
    this._findAndEnrichUrls = async (text) => {
      ...
      const newLinkPreviews = urls.filter((url2) => {
        const existingPreviews = this.previews;
        const existingPreviewLink = existingPreviews.get(url2) || existingPreviews.get(url2 + "/");
        return !existingPreviewLink;
      }).map(
        (url2) => ({
          og_scrape_url: url2.trim(),
    ...
    this.clearPreviews = () => {
      ...
      currentPreviews.forEach((preview, url2) => {
        if (_LinkPreviewsManager.previewIsDismissed(preview)) {
          newPreviews.set(url2, preview);
        }
      });
      this.state.partialNext({ previews: newPreviews });
    };
    this.updatePreview = (url2, preview) => {
      if (!url2) return;
      const existingPreview = this.previews.get(url2);
      const status = preview.status ?? this.previews.get(url2)?.status ?? "pending" /* PENDING */;
      ...
      this.state.partialNext({
        previews: new Map(this.previews).set(url2, {
          ...finalPreview,
          og_scrape_url: url2,
          status
        })
    ...
    this.dismissPreview = (url2) => {
      const preview = this.previews.get(url2);
      if (preview) {
        this.onLinkPreviewDismissed?.(preview);
        this.updatePreview(url2, { status: "dismissed" /* DISMISSED */ });
      }
    };
    this.composer = composer;
...
var Thread = class extends WithSubscriptions {
  constructor({
    ...
    this.throttledMarkAsRead = throttle2(
      () => this.markAsRead(),
...
var Channel = class {
  ...
  deleteFile(url2) {
    return this.getClient().delete(`${this._channelURL()}/file`, { url: url2 });
  }
  deleteImage(url2) {
    return this.getClient().delete(`${this._channelURL()}/image`, { url: url2 });
  }
  /**
  ...
  async updateMemberPartial(updates, options) {
    const url2 = new URL(`${this._channelURL()}/member`);
    if (options?.userId) {
      url2.searchParams.append("user_id", options.userId);
    }
  ...
  async _deleteReaction(messageID, reactionType, user_id) {
    ...
    const url2 = this.getClient().baseURL + `/messages/${encodeURIComponent(messageID)}/reaction/${encodeURIComponent(
      reactionType
    )}`;
    if (user_id) {
      return await this.getClient().delete(url2, { user_id });
    }
    return await this.getClient().delete(url2, {});
  }
  /**
...
// src/connection.ts
var import_isomorphic_ws = __toESM(require_node2());
...
var ThreadManager = class extends WithSubscriptions {
  ...
  constructor({ client }) {
    ...
    this.subscribeRecoverAfterConnectionDrop = () => {
      ...
      const throttledHandleConnectionRecovered = throttle2(
        () => {
...
var StreamChat = class _StreamChat {
  constructor(key, secretOrOptions, options) {
    ...
    this.doAxiosRequest = async (type, url2, data, options = {}) => {
      await this.tokenManager.tokenReady();
      const requestConfig = this._enrichAxiosOptions(options);
      try {
        let response;
        this._logApiRequest(type, url2, data, requestConfig);
        switch (type) {
          case "get":
            response = await this.axiosInstance.get(url2, requestConfig);
            break;
          case "delete":
            response = await this.axiosInstance.delete(url2, requestConfig);
            break;
          case "post":
            response = await this.axiosInstance.post(url2, data, requestConfig);
            break;
          case "postForm":
            response = await this.axiosInstance.postForm(url2, data, requestConfig);
            break;
          case "put":
            response = await this.axiosInstance.put(url2, data, requestConfig);
            break;
          case "patch":
            response = await this.axiosInstance.patch(url2, data, requestConfig);
            break;
          case "options":
            response = await this.axiosInstance.options(url2, requestConfig);
            break;
          default:
            throw new Error("Invalid request type");
        }
        this._logApiResponse(type, url2, response);
        this.consecutiveFailures = 0;
        return this.handleResponse(response);
      } catch (e) {
        e.client_request_id = requestConfig.headers?.["x-client-request-id"];
        this._logApiError(type, url2, e);
        this.consecutiveFailures += 1;
        if (e.response) {
          if (e.response.data.code === chatCodes.TOKEN_EXPIRED && !this.tokenManager.isStatic()) {
            ...
            return await this.doAxiosRequest(type, url2, data, options);
          }
    ...
    if (this.node && !this.options.httpsAgent) {
      this.options.httpsAgent = new import_https2.default.Agent({
        keepAlive: true,
        keepAliveMsecs: 3e3
      });
    }
  ...
  _logApiRequest(type, url2, data, config) {
    this.logger("info", `client: ${type} - Request - ${url2}`, {
  ...
  _logApiResponse(type, url2, response) {
    this.logger(
      "info",
      `client:${type} - Response - url: ${url2} > status ${response.status}`,
      {
  ...
  _logApiError(type, url2, error) {
    this.logger("error", `client:${type} - Error - url: ${url2}`, {
      tags: ["api", "api_response", "client"],
      url: url2,
      error
    });
  ...
  get(url2, params) {
    return this.doAxiosRequest("get", url2, null, { params });
  }
  put(url2, data) {
    return this.doAxiosRequest("put", url2, data);
  }
  post(url2, data) {
    return this.doAxiosRequest("post", url2, data);
  }
  patch(url2, data) {
    return this.doAxiosRequest("patch", url2, data);
  }
  delete(url2, params) {
    return this.doAxiosRequest("delete", url2, null, { params });
  }
  sendFile(url2, uri, name, contentType, user) {
    const data = addFileToFormData(uri, name, contentType || "multipart/form-data");
    if (user != null) data.append("user", JSON.stringify(user));
    return this.doAxiosRequest("postForm", url2, data, {
   ...
   */
  enrichURL(url2) {
    return this.get(this.baseURL + `/og`, { url: url2 });
  }
  /**
